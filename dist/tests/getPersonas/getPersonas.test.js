"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = y[op[0] & 2 ? "return" : op[0] ? "throw" : "next"]) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [0, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
Object.defineProperty(exports, "__esModule", { value: true });
var assert = require("assert");
var lodash_1 = require("lodash");
var cursor_1 = require("../../repoFactory/utils/cursor");
var GetOptions_1 = require("../../serviceFactory/utils/GetOptions");
var setup_1 = require("../utils/setup");
var values_1 = require("../utils/values");
describe('getPersonas', function () {
    var service = setup_1.default();
    var addTestPersonas = function () { return __awaiter(_this, void 0, void 0, function () {
        var NUM_PERSONAS, resultsPromise, results;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    NUM_PERSONAS = 4;
                    resultsPromise = lodash_1.times(NUM_PERSONAS, function (i) {
                        return service.createPersona({
                            name: "Dave " + i,
                            organisation: values_1.TEST_ORGANISATION,
                        });
                    });
                    return [4 /*yield*/, Promise.all(resultsPromise)];
                case 1:
                    results = _a.sent();
                    return [2 /*return*/, lodash_1.map(results, function (_a) {
                            var persona = _a.persona;
                            return persona;
                        })];
            }
        });
    }); };
    var fromFirstCursor = cursor_1.modelToCursor({
        model: {
            name: 'Dave 0',
        },
        sort: {
            name: 1,
        },
    });
    var getPersonaOptions = {
        direction: GetOptions_1.CursorDirection.FORWARDS,
        filter: {},
        limit: 2,
        maxScan: 0,
        maxTimeMS: 0,
        organisation: values_1.TEST_ORGANISATION,
        project: {},
        sort: {
            name: 1,
        },
    };
    it('Should get the 2 personas', function () { return __awaiter(_this, void 0, void 0, function () {
        var result, TWO;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, addTestPersonas()];
                case 1:
                    _a.sent();
                    return [4 /*yield*/, service.getPersonas(lodash_1.assign({}, getPersonaOptions, {
                            cursor: fromFirstCursor,
                        }))];
                case 2:
                    result = _a.sent();
                    TWO = 2;
                    assert.equal(result.edges.length, TWO);
                    assert.equal(result.edges[0].node.name, 'Dave 1');
                    assert.equal(result.edges[1].node.name, 'Dave 2');
                    return [2 /*return*/];
            }
        });
    }); });
});
//# sourceMappingURL=getPersonas.test.js.map