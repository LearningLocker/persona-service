...
export default (config: Config, collectionName: string) => {
  return async <T extends BaseModel>({
    ...
    const mongoCursor = await collection.find({})
      .filter(theFilter)
      .sort(sort)
      .project(project)
      .limit(limit + 1)
    ...
    const result2: PaginationResult<T> = {
      edges,
      pageInfo: {
        endCursor: !lastEdge ? undefined : lastEdge.cursor,
        hasNextPage: (direction === CursorDirection.BACKWARDS && !!cursor) ||
          direction === CursorDirection.FORWARDS && result.length > limit,
        hasPreviousPage: (direction === CursorDirection.FORWARDS && !!cursor) ||
          direction === CursorDirection.BACKWARDS && result.length > limit,
        startCursor: !firstEdge ? undefined : firstEdge.cursor,
      },
    };
