...
import { PERSONAS_COLLECTION, PERSONA_IDENTIFIERS_COLLECTION } from './utils/constants/collections';
...
export default (config: Config) => {
  return async ({personaId, organisation}: DeletePersonaOptions): Promise<void> => {
    ...
    const personaIdentifiersCollection = (await config.db).collection(PERSONA_IDENTIFIERS_COLLECTION);
